...
for i = 1:numel(anames) % This loop envelops the entire script to analyze each animal 1 at a time.
    ...
    fpath = {};
    g = [];
    for i = 3:sum(arrayfun(@(animalfilelist) ~isempty(animalfilelist.name),animalfilelist))
        g = append(animalfilelist(i).folder,'\',animalfilelist(i).name);
        fpath = [fpath g];
    end
    numfiles = numel(fpath);
    killoop = 0;
    while killoop == 1
        killoop = "THIS LOOP SHOULD NOc T RUN";
        ...
        clear sig2_pr_resid_r sig1_pr_resid_r sig2_br_resid_r sig1_br_resid_r...
            ...
            nVec_sig1_b nVec_sig1_p nVec_sig2_b nVec_sig2_p numfiles  nImgs...
    ...
    t = t./10;
    rng default
    sig1_denoised_resid = sin(2*pi.*(t)) + randn(size(t))*.1;
    sig2_denoised_resid = sin(2*pi*(39/40).*(t+0.5)) + randn(size(t))*.1;
    MMsig1_str = "signal 1";
    MMsig2_str = "Signal 2";
    ...
    for i = 1:length(rnglist) % Iterate through each of the windows of interest
        ...
        for j = 1:numel(sig1_denoised_resid(1,:)) % Iterate through every column within the matrix
            ...
            [r,lags] = xcorr(x,y,'coeff'); % This performs the xcorrelation
            lags = rot90(lags,-1);
            r = rot90(r,-1);
            r = atanh(r);
    ...
    for i = 1:xcorrindex
        ...
        xcorrmultioutmaxmatrix(i,:) = xcorrmultioutmaxima{1,i};
    end
    summaryMatrix{summarycounter} = xcorrmultioutmaxmatrix;
...
for i = 1:summarycounter
    xlswrite(outputfpath,summaryMatrix{i}, anames{i});
end

clear i j o r x y z xcorrindex  upperrng lowerrng rng names mpath...
    astring animalfilelist anames string lags g outputfpath rnglist rngnames...
