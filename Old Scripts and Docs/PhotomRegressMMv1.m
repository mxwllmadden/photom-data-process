...
for i = 1:numel(anames) % This loop envelops the entire script to analyze each animal 1 at a time.
    ...
    fprintf('Performing First Signal Regression!\n')
    ...
    sig1_denoised_resid = sig1_denoised_resid - mean(sig1_denoised_resid(601:4200,:));
    sig1_denoised_resid = sig2_denoised_resid - mean(sig2_denoised_resid(601:4200,:));
    ...
    for k=1:nImgs*binsize
        ...
        nVec_sig1(k,:)=[];
        std1_dn(k,:)=std(nVec_sig1(o:p,:));
        
        nVec_sig2=sig2_denoised_resid;
        nVec_sig2(k,:)=[];
        std2_dn(k,:)=std(nVec_sig2(o:p,:));
    end
